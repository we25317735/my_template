...
router.get(
  ...
  LineLogin.callback(
    ...
    async (req, res, next, token_response) => {
      console.log('line 回傳: ', token_response)
...
// 現有帳號綁定(式做)
// 綁定LINE帳號的路由
router.post('/bind-line', async (req, res) => {
  const { line_uid, userId } = req.body // 假設這是從前端發送的資料

  try {
    // 更新現有使用者的 LINE UID
    await User.update(
      { line_uid: line_uid }, // 更新的欄位
      { where: { id: userId } } // 條件
    )

    return res.json({ status: 'success', message: 'LINE帳號綁定成功' })
  } catch (error) {
    console.error(error)
    return res.json({ status: 'error', message: '綁定失敗' })
  }
})
